...
namespace UnityEditor
{
	public sealed class EditorGUI
	{
		...
		private static Enum EnumPopupInternal(Rect position, GUIContent label, Enum selected, GUIStyle style)
		{
			...
			num = EditorGUI.Popup(position, label, num, EditorGUIUtility.TempContent(nonObsoleteEnumData.displayNames), style);
			return (num >= 0 && num < nonObsoleteEnumData.values.Length) ? nonObsoleteEnumData.values[num] : selected;
		}
