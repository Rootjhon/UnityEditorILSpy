...
namespace UnityEditor
{
	public sealed class PluginImporter : AssetImporter
	{
		...
		internal void SetShouldOverridePredicate(Predicate<string> shouldOverridePredicate)
		{
			...
			{
				PluginImporter.s_shouldOverridePredicateMap[base.assetPath] = shouldOverridePredicate;
			}
			else if (PluginImporter.s_shouldOverridePredicateMap.ContainsKey(base.assetPath))
			{
				PluginImporter.s_shouldOverridePredicateMap.Remove(base.assetPath);
			}
		}
		...
		{
			bool result;
			if (PluginImporter.s_shouldOverridePredicateMap.ContainsKey(base.assetPath))
			{
				try
				{
					result = PluginImporter.s_shouldOverridePredicateMap[base.assetPath](base.assetPath);
					return result;
				}
				catch (Exception)
				{
					UnityEngine.Debug.LogWarning("Exception occurred while invoking ShouldOverridePredicate for " + base.assetPath);
				}
			}
			result = false;
			return result;
